...
public class JwtService{
    ...
    public Optional<Jwt> generateJwt(AccountCredentials accountCredentials) {
        try {
            final var jwt = JWT.create()
                    .withSubject(accountCredentials.getPhoneNumber().getPhoneNumber())
    ...
    public Optional<String> extractUserPhoneNumberFromJwt(Jwt jwt) {
        try {
    ...
    public Optional<String> extractRoleFromJwt(Jwt jwt) {
        try {
            DecodedJWT decodedJWT = JWT.decode(jwt.jwt());
            return Optional.ofNullable(decodedJWT.getClaim("role").asString());
        } catch (JWTDecodeException e) {
            return Optional.empty();
        }
    }
